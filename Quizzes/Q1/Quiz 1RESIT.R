...
sd(prawn[prawn$Diet=="Artificial",]$GrowthR) # same as in quiz
mean(prawn[prawn$Diet=="Natural",]$GrowthR) # same as in quiz
sd(prawn[prawn$Diet=="Natural",]$GrowthR) # same as in quiz

prawn<-read.table("prawn.txt", header=TRUE)
str(prawn)
prawn_Artificial<-subset(prawn, Diet=="Artificial")
prawn_Natural<-subset(prawn, Diet=="Natural")
mean(prawn_Artificial$GrowthR)
sd(prawn_Artificial$GrowthR)
mean(prawn_Natural$GrowthR)
sd(prawn_Natural$GrowthR)

or this way

mean(mussels[mussels$temperature=="LEN5D",]$length)

sd(mussels[mussels$temperature=="LEN5D",]$length)

mean(mussels[mussels$temperature=="LEN15D",]$length)

sd(mussels[mussels$temperature=="LEN15D",]$length)
