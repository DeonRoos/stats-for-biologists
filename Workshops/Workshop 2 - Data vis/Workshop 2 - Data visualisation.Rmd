...
```{r, include=answer_sheet, class.source="bg-success", eval = F}
# Yes - we're likely to break the assumption of addidivity - I.e. the length of a boto is more than just the additive effect of sex and drought independently. It looks like drought impacts males more than female. The contrast would be that if imagined slopes were identical for both females and males.
```
...
We're going to do something that's, hopefully, fun but also instructive. On MyAberdeen, you'll find a dataset called `abdn_rent.txt`. This dataset contains $> 100$ records (i.e. observations or samples) of rent prices for properties listed in Aberdeenshire on the [ASPC](https://www.aspc.co.uk/) website. In addition to the rent price, 11 additional variables were recorded as well. The variables are:
...
5.  `FloorArea` - the total floor area of the property (measured in m$^2$)

6.  `EPC` - th Energy Performance Certificate (A is highly energy efficient, G is terrible)

7.  `Tax` - the council tax band (which is based on the value of the property on 1st of April 1991; A is the cheapest council tax, H is the most expensive)

8.  `DateAdded` - the date when the record was *added to the dataset* (note this is not necessarily when the property was listed on the site but when it was extracted)

9.  `PropertyURL` - web link for the property listing

10. `Address` - the property name or address

11. `Latitude` - the latitude coordinates of the property (note latitude is North to South - think of a "ladder" going up and down, which sounds kinda like "latter")

12. `Longitude` - the longitude coordinates of the property (note longitude is East to West - think of "the *long* way around")

13. `Commute_time_minutes` - the estimated time it takes to walk to the University of Aberdeen from this property (estimates are extracted using the Google Maps API).

Spend a bit of time familiarising yourself with the data, i.e. do a quick EDA to make sure there's nothing weird. If you find what you think is a mistake, well in this case, you can check and correct yourself, as you have the *original* data because of the web link! A small note, however, is that sometimes ASPC will remove listings. We'll come back to this dataset later in the course where you'll be using it to predict how much rent a landlord *should* be asking for rent, so it's not wasted time getting to know the data.
...
p5 <- ggplot(abdn) +
  geom_point(aes(x = FloorArea, y = Price, colour = Bedrooms)) +
  theme_classic() +
...
Now for the part I'm most excited about. Do the same as above, but this time, make it a miserably ugly figure. It still needs to *contain* the information (i.e. no blank figures) and be *somewhat* readable. If you need inspiration, search for "graph crimes", or "chart crimes" in your search engine - there are *lots* of examples you can use for "inspiration".

Once you're happy with your masterpieces, add your figures onto the MyAberdeen discussion board, and we'll have an "Art Exhibit" to finish the workshop in the last 15 minutes :D.
...
ggplot(abdn) +
  ...
  theme(
    ...
    legend.background = element_rect(fill = "cyan", colour = "black", linewidth = 2),
    legend.title = element_text(size = 15, colour = "brown", face = "italic"),
    legend.text = element_text(size = 10, colour = "darkred"),
    strip.background = element_rect(fill = "grey", colour = "black", linewidth = 1.5),
...
While it's important to be able to create figures, it's also important to be able to *interpret* them. To that end, below are a series of figures using the flat price data. For each, answer what your interpretation of that figure is.

## Question Set Four

Question 1. Which of the following statements are true of the histogram below?

1.  Most rental prices are below £1000 per month.
2.  The most expensive properties are more than £2500 per month.
3.  The spread of rental price is skewed towards properties at roughly £750 per month.
4.  Each band represents the number of properties in successive £100 increments

```{r}
ggplot(abdn) +
  geom_histogram(aes(x = Price), 
                 colour = "white", binwidth = 25) +
  theme_minimal()
```

Question Three. What do you learn about estate agent behaviour based on the histogram above?

```{r}
# It looks like when the property is <£1,000 per month, they round to the nearest
# £50, occasionally rounding to the nearest £25. However, once the property is over
# £1,000 per month, they tend to round to the nearest £100.
```

Question Two. What do you learn about a possible relationship between longitude and rent in the figure below?

1.  Price becomes more variable as properties go from west (ca. -3 longitude) to east

```{r}
ggplot(abdn) +
  geom_point(aes(x = Longitude, y = Price)) +
  theme_minimal()
```
