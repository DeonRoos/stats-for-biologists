...
Regardless, the Q-Q plot helps us assess this assumption by comparing the distribution of the residuals to am "idealised" normal distribution. If the residuals follow a straight line in the Q-Q plot, this suggests they are approximately normally distributed.
...
```{r, include=answer_sheet, class.source="bg-success", eval = F}
# Load data
beetles <- read.table("H:/BI3010/Workshop3/beetles.txt", header = TRUE)
```
...
# Model fit
beetles_mod <- lm(offspring ~ temperature, data = beetles)

# Diagnostics
plot(beetles_mod)
# Diagnostics look final except for Residuals vs Fitted
# There seems to be a clear pattern in the residuals
# Negative to positive to negative again

# Plot predictions (with raw data as points)
beetles$fit <- predict(beetles_mod)
ggplot(beetles) +
  geom_point(aes(x = temperature, y = offspring)) +
