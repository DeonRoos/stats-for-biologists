...
library(ggplot2)
rabbit <- read.table("Data/rabbits.txt", header = TRUE)
rabbit$mongoose[7] <- "Present"
rabbit$weight[rabbit$weight > 3500] <- rabbit$weight[rabbit$weight > 3500] - 1000
...
**Mistake 2:** They cannot remember which rabbit, but they accidentally added 1000 to its weight.
...
There is one rabbit much heavier than all others, well above 3500 g. We can use that as a condition to identify it and subtract 1000:

```r
rabbit[rabbit$weight > 3500, ]
...
$$\mu_i = 2695.6 + (-28.5) \times \text{Present}_i$$

The first blank is 2695.621 ($\beta_0$) and the second is &minus;28.528 ($\beta_1$). Rounding to one decimal place is appropriate here given that weights are recorded to a tenth of a gram and the estimated difference is small relative to the rabbits' body weight.
</div>
:::
...
<div class="mc-options">
  <label><input type="radio" name="qs4-q2" data-correct="false"> 2695.6 g</label>
  <label><input type="radio" name="qs4-q2" data-correct="false"> 2724.1 g</label>
  <label><input type="radio" name="qs4-q2" data-correct="true"> 2667.1 g</label>
  <label><input type="radio" name="qs4-q2" data-correct="false"> 28.5 g</label>
</div>
<button class="reveal-btn" onclick="revealAnswer(this)">Show Answer</button>
<div class="instructor-answer">
Substitute `Present = 1`:

$$\mu_i = 2695.6 + (-28.5) \times 1 = 2667.1 \text{ g}$$
...
<div class="mc-options">
  <label><input type="radio" name="qs4-q3" data-correct="false"> 2667.1 g</label>
  <label><input type="radio" name="qs4-q3" data-correct="false"> 2724.1 g</label>
  <label><input type="radio" name="qs4-q3" data-correct="true"> 2695.6 g</label>
...
$$\mu_i = 2695.6 + (-28.5) \times 0 = 2695.6 \text{ g}$$
...
<button class="reveal-btn" onclick="revealAnswer(this)">Show Answer</button>
<div class="instructor-answer">
Taken at face value, yes: the model estimates that rabbits are about 29 g lighter where mongoose are present (2695.6 g when absent, 2667.1 g when present). But before reading anything into that, look at the *size* of the effect. Amami rabbits weigh on average around 2.7 kg, so a difference of roughly 29 g is only about 1% of body weight. Biologically, that is tiny. It's hard to see how a difference that small would tell us anything meaningful about whether mongoose are harming the rabbits.

This is a judgement about *biological* importance, not one about statistical significance (i.e. we aren't using P-values here). Tools like P-values are what a lot of people reach for to decide if something is important or not, despite being told for decades not to, rather than just thinking about it for themselves. Here, I don't care if 30 g is statistically significant. It's biologically *unimportant*. A 1% change in body mass is utterly biologically trivial.
</div>
:::
...
<button class="reveal-btn" onclick="revealAnswer(this)">Show Answer</button>
<div class="instructor-answer">
29 g is pretty small for a species that weighs on average around 2.7 kg, only about 1% of body weight. Combined with the wide uncertainty and the unresolved question of whether mongoose are even absent from Tokunoshima, we're not really sure what we've learned here.
...
Then refit and rerun `ggpredict()`. In truth, I've never found fitting a model with characters to be especially problematic, though some packages won't allow it at all. So I generally do convert my categorical variables to factors before fitting models. Whether you want to or not is up to you, but given how little effort it takes, I lean towards just converting to `factor`.
